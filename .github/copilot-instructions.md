...
## Invariants
- Plate: `^[A-Z]{2}[0-9]{1,2}[A-Z]{1,3}[0-9]{4}$`; country code: `+91`.
- QR length: 6; normalized alert issue: `issue_type="Emergency"`.
- Anti-spam: 3 scans/hour/tag. OTP: 6 digits, 10-minute expiry, 3 attempts, 60-second cooldown.
- JWT session: 72 hours. Contact form cooldown: 60 seconds per source IP.
- Rules constants: `BENCHMARK_PRICE=90.50`, `TANK_CAPACITY=350.0L`, `EXPECTED_KML=4.0`.

## Failure Stop Rule
- Stop after two unsuccessful attempts in the same file or validation slice, or when the same error repeats. State the root cause in two sentences and ask for clarification only when genuinely blocked.
