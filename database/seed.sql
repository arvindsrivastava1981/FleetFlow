...
INSERT INTO trips (trip_code, vehicle_id, vehicle_no, driver_name, driver_phone, advance_amount, start_odo, current_odo, end_odo, status, origin, destination, completed_at, settled_at, created_by, driver_user_id)
VALUES (
    'TRIP-104',
    (SELECT id FROM vehicles WHERE vehicle_number = 'UP93AT1234'),
    'UP93AT1234', 'Rajesh Patel', '+91 98765 33344', 30000.00, 103550.00, 104120.00, 104120.00,
...
VALUES (
    'TRIP-105',
    (SELECT id FROM vehicles WHERE vehicle_number = 'UP93AT1234'),
    'UP93AT1234', 'Vikram Singh', '+91 98765 44455', 28000.00, 104120.00, 104680.00, 104680.00,
...
-- manager1	manager123
-- driver1	driver123
-- (Users are created above the vehicle inserts because vehicles reference users)
-- ----------------------------------------------------------------------------
