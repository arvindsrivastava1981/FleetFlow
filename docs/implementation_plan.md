...
| --- | --- | --- | --- |
| G1 | **Real WhatsApp Cloud API** integration (inbound media webhooks, outbound message templates, session binding by WhatsApp number). | Only a browser **simulator** (`DriverWhatsApp.jsx` → `POST /api/v1/expenses` + `ManagerWhatsApp.jsx` → `GET /api/v1/whatsapp/escalations` / `POST /api/v1/expenses/{id}/action`) posts to the JSON API. `requirements.txt` lists `requests`/`httpx` but nothing calls WhatsApp. | No webhook endpoint, no template handling, no real driver WhatsApp flow. |
