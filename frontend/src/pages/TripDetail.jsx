...
export default function TripDetailPage() {
  ...
  async function settleTrip() {
    const endOdo = Number(settleOdo);
    const startOdo = Number(data?.trip?.start_odo || 0);
    if (endOdo < startOdo) {
      toast.error(
  ...
  const showSettle =
    canSettle && (trip?.status === "ACTIVE" || trip?.status === "COMPLETED");
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Expense entry now lives on the dedicated full-screen form at /log */}
      {trip && trip.status === "ACTIVE" && canSettle && (
        <Link
          to={`/trips/${tripCode}/log`}
          className="btn-primary flex items-center justify-center gap-2 rounded-xl py-3 text-base"
        >
          ⚡ Log Expense (quick entry)
        </Link>
      )}
        ...
        <div className="card p-4 space-y-3">
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">
              All expenses must be APPROVED or REJECTED first. Settling closes
              the trip and locks the settlement ledger.
            </p>
          </div>
