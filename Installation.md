...
ADMIN_PASSWORD=change-me       # required in production (see below)
...
| `ADMIN_PASSWORD` | In prod  | Admin login password. In production, `admin123` is **refused** — you must set an explicit strong value. |
...
`DATABASE_URL` + `ADMIN_PASSWORD` provided as manual secrets in the Render dashboard
