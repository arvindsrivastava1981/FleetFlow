...
- **Onboarding wizard:** `POST /api/v1/fleets/onboard` (`backend/app/api/v1/onboard.py`) — creates fleet + owner Trip Manager + trial (+ optional vehicle/driver) in one transaction; returns inline `payment_url` for paid plans (G2).
...
- **[backend/app/main.py](/backend/app/main.py)** — the modular FastAPI factory; the **only entry point** (migration complete).
  ...
  - **Static SPA mount (Phase 1):** after all routers, if `frontend/dist/` exists, `main.py` mounts `/assets` statically and registers an **explicit root route `GET|HEAD /`** (serves `index.html`) plus a **catch-all `GET /{path:path}`** that serves real files from `dist/` or falls back to `index.html` (SPA routing). It only activates when `frontend/dist/index.html` is present — otherwise the server is pure-API/HTML. Explicit API/HTML routes always win because they register before the catch-all. **All responses** (SPA HTML, hashed assets, every `/api/v1` endpoint, webhook, `/healthz`) carry `Cache-Control: no-store, no-cache, must-revalidate, max-age=0` + `Pragma: no-cache` + `Expires: 0` via an outer HTTP middleware, so no layer caches anything in production.
...
- `compute_settlement` (`backend/app/services/audit/cash.py`) derives `advance_amount` and `driver_batta` from the trip's `APPROVED` `CASH_ADVANCE`/`DRIVER_SALARY` ledger rows, falling back to the `trips` columns (`advance_amount`, `driver_batta_amount`) for legacy rows predating the auto-post. Net = advance + goods_income − (road expenses + driver batta).
